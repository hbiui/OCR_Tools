...
import { analyzePosterImage } from './services/geminiService';
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        try {
          ...
          if (currentProviderAtStart !== OCRProvider.GEMINI) {
            setImages(prev => prev.map(img => img.id === processingId ? { ...img, currentStep: 'OCR 处理中' } : img));
            ...
            setImages(prev => prev.map(img => img.id === processingId ? { ...img, currentStep: 'AI 识别文本' } : img));
          }
          ...
          setImages(prev => prev.map(img => img.id === processingId ? { ...img, currentStep: 'AI 分析中' } : img));
          const result = await analyzePosterImage(base64Data, terminology, ocrText);
          ...
          setImages(prev => prev.map(img => img.id === processingId ? { ...img, currentStep: '结果生成中' } : img));
          ...
          setImages(prev => prev.map(img => img.id === processingId ? { ...img, status: 'error', currentStep: undefined } : img));
        }
      }
    } finally {
...
使用的 OCR 引擎: ${image.ocrProvider ? (OCR_PROVIDER_INFO[image.ocrProvider as keyof typeof OCR_PROVIDER_INFO]?.name || image.ocrProvider) : '未知'}
原始提取文本:
${image.rawOcrText || '未提取到文本'}
    ...
    <div className="min-h-screen">
      <nav className="glass-panel sticky top-4 mx-4 md:mx-8 z-[100] px-8 py-5 flex justify-between items-center rounded-3xl border-white/10 mt-4 transition-all">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <span className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] block mt-1">文字OCR检测工具</span>
          </div>
      ...
      <main className="container mx-auto px-4 md:px-8 py-10 grid grid-cols-12 gap-x-8 gap-y-10 items-stretch">
        <div className="col-span-12 lg:col-span-4 space-y-8 flex flex-col">
          ...
          >
            ...
            <h3 className="text-xl font-extrabold text-white mb-2">{isDragging ? '放置图片以添加' : '上传图片'}</h3>
            <p className="text-sm text-slate-500 leading-relaxed font-medium">支持多维度批量导入，AI 实时识别安防语义</p>
          </div>

          <div className="glass-panel rounded-[2rem] p-6 min-h-[500px] flex-1 flex flex-col relative overflow-hidden">
            <div className="flex flex-col gap-4 mb-6 px-2">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-500">图片列表 ({images.length})</h3>
                </div>
                <div className="flex items-center gap-3">
                  {images.length > 0 && (
                    <>
            ...
            <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
              {images.map((img, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative shrink-0 w-14 h-14 rounded-2xl overflow-hidden shadow-lg ml-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-xs font-bold truncate text-white">{img.file.name}</p>
              ...
              {images.length === 0 && (
                <div className="flex flex-col items-center justify-center py-24 opacity-20 grayscale">
                  ...
                  <span className="text-[10px] font-black tracking-widest uppercase">无输入数据</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className={`col-span-12 lg:col-span-8 flex flex-col ${currentIndex === null ? 'h-full' : 'h-auto'}`}>
          {currentIndex !== null && images[currentIndex] ? (
            <div className="animate-in fade-in slide-in-from-bottom-6 duration-1000 space-y-10 flex-1">
              <div className="glass-panel rounded-[3rem] overflow-hidden grid grid-cols-1 xl:grid-cols-2 min-h-[600px] border-white/5">
                ...
                >
                  ...
                  <div 
                    className="relative z-10 inline-flex flex-col items-center justify-center mx-auto shadow-[0_40px_80px_rgba(0,0,0,0.6)] select-none"
                  ...
                  >
                    <img 
                      ...
                      className="max-h-[550px] h-auto w-auto max-w-full block object-contain rounded-2xl pointer-events-none" 
                    />
                    ...
                    <>
                      <div className="flex items-center justify-between">
                        <div className="space-y-4">
                          <h2 className="text-4xl font-black text-white tracking-tight">分析结果</h2>
                          <div className="flex items-center gap-3">
                            <span className={`text-[10px] font-black text-white px-3 py-1 rounded-full uppercase shadow-lg ${images[currentIndex].result?.isProfessional ? 'bg-emerald-500' : 'bg-rose-500'}`}>
                              {images[currentIndex].result?.isProfessional ? '推荐发布' : '需要修改'}
                            </span>
                            {images[currentIndex].ocrProvider && (
                              <span className="text-[9px] font-black text-slate-500 bg-slate-500/10 px-2.5 py-1 rounded-lg border border-white/5 uppercase tracking-[0.2em] flex items-center gap-1.5">
                                ...
                                引擎: {OCR_PROVIDER_INFO[images[currentIndex].ocrProvider as keyof typeof OCR_PROVIDER_INFO]?.name || images[currentIndex].ocrProvider}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      {images[currentIndex].isLowRes && (
                        <div className="p-5 bg-amber-500/5 border border-amber-500/20 rounded-2xl flex items-start gap-4">
                          <div className="w-10 h-10 bg-amber-500/20 rounded-xl flex items-center justify-center shrink-0">
                            <svg className="w-6 h-6 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                          </div>
                          <div>
                            <p className="text-xs font-black text-amber-500 uppercase tracking-widest mb-1">分辨率限制提醒</p>
                            <p className="text-[11px] text-slate-500 font-bold leading-relaxed">检测到海报尺寸较小，OCR 识别可能存在像素丢失。为了获得最专业的语义纠错效果，建议设计师上传 1080P 以上的分辨率原图进行分析。</p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="space-y-8">
                        <div className="flex items-center justify-between">
                          <h4 className="text-[11px] font-black text-indigo-400 uppercase tracking-[0.4em]">深度优化建议清单</h4>
                          <span className="text-[10px] font-bold text-slate-600">共发现 {images[currentIndex].result?.errors.length} 项风险</span>
                        </div>
                        ...
                        {images[currentIndex].result?.errors.length === 0 ? (
                          <div className="p-12 bg-emerald-500/5 rounded-[2.5rem] border border-emerald-500/20 text-emerald-400 text-center space-y-6">
                            ...
                            <div>
                                <p className="font-black text-2xl mb-2">完美的安防表达</p>
                                <p className="text-sm opacity-60 leading-relaxed max-w-xs mx-auto">此海报展示了极高的专业水准，所有术语与上下文完全符合国际出口标准。</p>
                            </div>
                          </div>
                        ) : (
                                  ...
                                  <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex flex-col">
                                      <span className="text-xs font-black uppercase text-slate-400 tracking-wider">
                                        {error.type === 'spelling' ? '拼写修正' : error.type === 'grammar' ? '语法精炼' : error.type === 'terminology' ? '行业术语' : '风格建议'}
                                      </span>
                                ...
                                <div className="p-6 pt-0 space-y-6">
                                  <div className="flex items-start gap-4 bg-emerald-500/10 p-5 rounded-2xl border border-emerald-500/20 shadow-inner">
                                    ...
                                    <div className="flex-1">
                                      <p className="text-xs font-black uppercase text-emerald-500/50 mb-1 tracking-widest">推荐方案</p>
                                  ...
                                  <div className="text-sm text-slate-400 leading-relaxed pl-4 border-l-2 border-slate-800 font-medium italic">
                                    <p className="text-[10px] font-black uppercase text-slate-600 mb-2 tracking-widest">技术分析依据</p>
                                    {error.explanation}
                                  </div>
                                  {error.alternatives && error.alternatives.length > 0 && (
                                    <div className="flex flex-wrap gap-2 pt-2">
                                      {error.alternatives.map((alt, i) => (
                                        <span key={i} className="px-3 py-1 bg-white/5 border border-white/5 rounded-lg text-[11px] text-slate-500 font-bold">
                                          备选: {alt}
                                        </span>
                                      ))}
                                    </div>
                      ...
                      {images[currentIndex].status === 'processing' ? (
                        <>
                          <div className="relative w-40 h-40">
                             ...
                             <div className="absolute inset-6 border-[4px] border-cyan-500/20 border-b-transparent rounded-full animate-spin direction-reverse duration-[2s]"></div>
                          ...
                          <div className="space-y-6 max-w-sm">
                            <h3 className="text-3xl font-black text-white tracking-tighter">AI 神经元深度分析</h3>
                          ...
                          <div className="w-80 space-y-3">
                             <div className="h-2 bg-slate-800 rounded-full overflow-hidden shadow-inner border border-white/5">
                                <div className="h-full bg-gradient-to-r from-indigo-500 to-cyan-400 shadow-[0_0_20px_#6366f1] animate-progress" style={{width: '60%'}}></div>
                             </div>
                          </div>
                        </>
                        ...
                        <div className="space-y-8 flex flex-col items-center">
                            ...
                            <div className="space-y-2">
                                <h3 className="text-2xl font-black text-white">分析中断</h3>
                                <p className="text-slate-500 font-medium">可能由于网络波动或 API 限制导致。请检查配置或重试。</p>
                            </div>
                            {!isProcessing && <Button variant="primary" onClick={() => retryImage(images[currentIndex].id)}>立即重试</Button>}
                        </div>
                      ) : (
                        <div className="space-y-10 opacity-30 grayscale">
                          ...
                          <p className="text-slate-500 font-black uppercase tracking-[0.5em] text-sm">等待分析</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {images[currentIndex].status === 'processing' ? (
                     <div className="flex flex-col gap-5">
                        <div className="flex gap-2.5">
                          <span className="w-1.5 h-4 bg-indigo-500 animate-pulse delay-75"></span>
                          <span className="w-1.5 h-4 bg-indigo-500 animate-pulse delay-150"></span>
                          <span className="w-1.5 h-4 bg-indigo-500 animate-pulse delay-300"></span>
                        </div>
                        <span className="italic text-slate-600 font-sans font-medium">正在构建多模态语义图谱，提取像素点背后的字符逻辑...</span>
                     </div>
                  ) : (images[currentIndex].rawOcrText || (images[currentIndex].status === 'error' ? "分析异常..." : "等待分析数据..."))}
                </div>
              </div>
            </div>
          ) : (
            <div className="h-full flex flex-col items-center justify-center glass-panel rounded-[4rem] border-2 border-dashed border-white/5 group hover:border-indigo-500/20 transition-all duration-1000 placeholder-glass-panel">
               ...
               <h2 className="text-4xl font-black text-slate-600 group-hover:text-slate-400 transition-colors duration-1000 tracking-tighter">等待图片上传分析...</h2>
               <p className="text-slate-500 mt-6 text-center max-w-sm leading-relaxed font-bold uppercase text-xs tracking-widest opacity-60">选择一个图片进行分析</p>
            </div>
          )}
        </div>
      ...
      {showConfig && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/95 backdrop-blur-3xl animate-in fade-in duration-500">
          <div className="glass-panel w-full max-w-4xl rounded-[3rem] p-12 border-white/10 shadow-[0_0_150px_rgba(79,70,229,0.3)] animate-in zoom-in-95 duration-500">
