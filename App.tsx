...
import { performCloudOcr } from './services/cloudOcrService';
...
const App: React.FC = () => {
  ...
  const updateGlobalConfig = () => {
    if (typeof window !== 'undefined') {
      window.__GUARDVISION_CONFIG__ = {
        geminiApiKey: config.provider === OCRProvider.GEMINI ? config.apiKey : undefined
      };
    }
  };
  ...
  const analyzeWithRetry = async (base64Data: string, terms: SecurityTerm[], ocrText?: string, maxRetries = 3) => {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        return await analyzePosterImage(base64Data, terms, ocrText, config.provider);
      } catch (err) {
        lastError = err;
          ...
          if (currentProviderAtStart !== OCRProvider.GEMINI) {
            ...
            try {
              if (currentProviderAtStart === OCRProvider.LOCAL || currentProviderAtStart === OCRProvider.ESEARCH) {
                ...
                // 云函数OCR - 注意：这里不再需要前端传递API密钥
                // API密钥已在云函数环境变量中配置
                const result = await performCloudOcr(currentProviderAtStart, base64Data, config.language);
                ocrText = result.text;
              }
              ...
              // 如果OCR失败，尝试直接使用Gemini分析
              setImages(prev => prev.map(img => img.id === processingId ? { 
                ...img, 
                currentStep: 'OCR失败，尝试直接AI分析' 
              } : img));
  ...
  const renderHighlightedText = useMemo(() => {
    ...
    // 根据错误项的位置或文本进行分割。由于是纯文本，我们通过关键词查找
    // 这种方式在文本量大且关键词重复时可能不够精准，但在 2026 AI 框架下是标准交互。
      ...
      <nav className="glass-panel sticky top-4 mx-4 md:mx-8 z-[100] px-8 py-5 flex justify-between items-center rounded-3xl border-white/10 mt-4 transition-all">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <span className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] block mt-1">AI安防术语校对系统</span>
          </div>
          ...
          {activeSidebarTab === 'image' ? (
            ...
            <div className="glass-panel rounded-[2rem] p-8 space-y-6 flex flex-col border border-white/5 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-500">
               <div>
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed font-medium">直接粘贴文案或产品参数，AI 将针对行业词库进行深度纠错与改写推荐。</p>
               </div>
               <textarea 
            ...
            <div className="animate-in fade-in slide-in-from-bottom-6 duration-1000 space-y-10 flex-1">
              <div className="glass-panel rounded-[3rem] overflow-hidden grid grid-cols-1 xl:grid-cols-2 min-h-[600px] border-white/5">
                ...
                >
                  ...
                  <div 
                    className={`relative z-10 inline-flex flex-col items-center justify-center mx-auto shadow-[0_40px_80px_rgba(0,0,0,0.6)] select-none w-full h-full max-w-full max-h-full ${images[currentIndex].file ? '' : 'overflow-y-auto custom-scrollbar p-10 bg-slate-900/40 rounded-[2rem] border border-white/5'}`}
                        ...
                        {images[currentIndex].result?.errors.length === 0 ? (
                          <div className="p-12 bg-emerald-500/5 rounded-[2.5rem] border border-emerald-500/20 text-emerald-400 text-center space-y-6">
                            ...
                            <div>
                                ...
                                <p className="text-sm opacity-60 leading-relaxed max-w-xs mx-auto">此图片或文案展示了极高的专业水准，所有表达均符合国际出口技术规范。</p>
                            </div>
                          </div>
                        ) : (
                ...
                <div className="p-8 bg-indigo-500/5 rounded-[2rem] border border-indigo-500/10 shadow-inner">
                   {config.provider === OCRProvider.GEMINI ? (
                     <div className="space-y-4">
                       ...
                       <p className="text-xs text-amber-300 leading-relaxed font-bold uppercase tracking-wider">
                         *提示：如需使用云函数中转的Gemini，请在Vercel环境变量中配置GEMINI_API_KEY
                       </p>
                     </div>
                   ) : (
                     <div>
                       ...
                       {config.provider !== OCRProvider.LOCAL && config.provider !== OCRProvider.ESEARCH && (
                         <p className="text-xs text-amber-300 leading-relaxed font-bold uppercase tracking-wider mt-4">
                           *商业OCR API将通过云函数安全中转，API密钥请配置在Vercel环境变量中
                         </p>
                       )}
                     </div>
                   )}
