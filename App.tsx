        ...
        try {
          ...
          if (currentProviderAtStart !== OCRProvider.GEMINI) {
            ...
            if (currentProviderAtStart === OCRProvider.LOCAL || currentProviderAtStart === OCRProvider.ESEARCH) {
              ...
              // 针对 BAIDU/WECHAT/ALIBABA，由于纯前端跨域限制，无法直接调用。
              throw new Error(`${info.name} 由于 CORS 策略限制无法在前端直接调用，请切换为离线引擎或原生 AI 代理。`);
            }

            if (!ocrText || ocrText.trim() === "") {
              throw new Error(`${info.name} 文本识别失败，请检查图片或 API 额度。`);
            }
          }
