    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries - 1) {
          await new Promise(resolve => setTimeout(resolve, 1500 * (attempt + 1))); // 指数级退避
        }
      }
    }
    throw lastError;
    ...
    try {
      for (let i = 0; i < images.length; i++) {
        ...
        try {
          ...
          setImages(prev => prev.map(img => img.id === processingId ? { ...img, currentStep: 'AI 分析中 (支持重试)' } : img));
                      ...
                      {images[currentIndex].status === 'processing' ? (
                        ...
                        <div className="space-y-8 flex flex-col items-center">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-slate-500 font-medium">由于网络波动或 API 超时导致。系统已尝试自动重试，请检查网络或配置后重试。</p>
                            </div>
